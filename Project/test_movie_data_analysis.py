...
Each test function is designed to check the behavior of specific methods
within the MovieDataAnalyzer class
under different conditions and input values.

Tested Methods:
- movie_type: Tests for handling invalid types, negative values, zero values,
and large values.
- actor_distributions: Tests for handling invalid gender types, 
invalid height types, and invalid height ranges.

The tests ensure that the methods in the MovieDataAnalyzer class raise 
appropriate exceptions
(TypeError, ValueError) when provided with invalid input and return expected
results for valid input.

Usage:
- To run these tests, use the pytest framework by executing `pytest` in the terminal.
"""
import pytest
...
def test_actor_distributions_invalid_gender():
    ...
    when the gender parameter is provided with an invalid type (e.g., an integer).
...
def test_actor_distributions_invalid_height_type():
    """
    Test that the actor_distributions function raises a TypeError when an invalid type 
    is provided for the max_height parameter.

    This test ensures that the function correctly handles the case where the max_height 
    parameter is given a string value instead of a numeric value.

    Raises:
        TypeError: If the max_height parameter is not of a numeric type.
    """
    with pytest.raises(TypeError):
        analyzer.actor_distributions(gender="M", max_height="tall", min_height=1.5)
...
def test_actor_distributions_invalid_height_range():
    ...
    """
    with pytest.raises(ValueError):
        analyzer.actor_distributions(gender="M", max_height=1.0, min_height=1.5)
...
def test_actor_distributions_invalid_height_values():
    ...
    when provided with invalid height ranges. Specifically, it tests the following cases:
    ...
    with pytest.raises(ValueError):
        analyzer.actor_distributions(gender="M", max_height=2.5, min_height=3.5)
...
def test_actor_distributions():
    ...
    distribution of"
    """
    result = analyzer.actor_distributions(gender="M", max_height=2.0, min_height=1.5)
